 ...
 * @param cwd         Working directory for content replacements (defaults to process.cwd()).
...
export async function execute(
  ...
  if (!dryRun && outputConfig.contentReplacements && outputConfig.contentReplacements.length > 0) {
    await applyContentReplacements(cwd ?? process.cwd(), outputConfig.contentReplacements);
  }
